const buttonEmbed = (function() {
  const baseUrl = "//www.reddit.com";
  const apiUrl = "//buttons.reddit.com";
  const logo = $q("a.logo");
  const up = $q("a.up");
  const down = $q("a.down");
  const submission = $q("a.submission-details");
  ...
  function getQueryParams() {
    const params = {};
    const segments = window.location.search.substring(1).split("&");
  ...
  function safeColor(colorString) {
    const match = colorString.match(/([A-F0-9]{6}|[A-F0-9]{3})/i);
    if (match) {
      return "#" + match[0];
    }
    return null;
